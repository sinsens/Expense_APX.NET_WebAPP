 ...
 * @使用说明: http://freshflower.iteye.com/blog/1606222
 * @version 2.0
 *********************************************/
var JTC = (function() {
	var JTC = function() {},
		...
		lang = {
			monthText: {
				zh: ['⼀月', '二月', '三月', '四月', '五月', '六月', '七月', '八月', '九月', '十月', '十⼀月', '十二月'],
		...
		util = {
			...
			formatDate: function(date, format) {
				...
				for(var key in lang) {
					if(new RegExp('(' + key + ')').test(format)) {
						format = format.replace(RegExp.$1, RegExp.$1.length == 1 ? lang[key] : ('00' +
							lang[key]).substr(('' + lang[key]).length));
					}
				}
				return format;
			},
			...
			checkDateRange: function(yyMMdd) {
				var set = config['set'];
				if(set.maxDate == 0 && set.minDate == 0) {
					return true;
				}
				if(set.minDate == 0) {
		...
		events = {
			...
			yearSelector: function(table, start, rep) {
				...
				if(rep == 1) {
					...
					$(objid[0]).onclick = function() {
						events.yearSelector(table, start - 10, 1);
					};
					$(objid[1]).onclick = function() {
						events.yearSelector(table, start + 10, 1);
					};
					return;
				}
				...
				for(var i = 0; i < 5; i++) {
					...
					for(var j = 0; j < 2; j++) {
						...
						var cellStyle = 'width:37px;;height:20px; text-align:center;font-size:12 px;cursor: pointer;';
				...
				cell.colSpan = 2;
				var bnStyle = 'border:0px;background-color:#FFFFFF;width:24px;height:20px; margin:0px 0 px;';
			...
			hideSelector: function(flag) {
				if(flag == 2) {
					if($(config.monthSelector)) {
						$(config.monthSelector).style.display = 'none';
					}
				} else {
					if($(config.yearSelector)) {
						$(config.yearSelector).style.display = 'none';
					}
				}
			},
			hideLayout: function() {
			...
			mouseDown: function(ev) {
				...
				var ymFlag = 0;
				if($(config.yearSelectCtrl)) {
					ymFlag = 1;
				}
				if($(config.monthSelectCtrl)) {
					ymFlag = 1;
				}
		...
		dialog = function() {
			this.getHeaderPanel = function() {
				...
				var bn = util.createButton('<', bnStyle, function() {
						events.turnMonth(-1);
					},
					'bn_preMonth');
				cell.appendChild(bn);
				...
				cell.onclick = function() {
					events.showSelector(2);
				};
				cell = row.insertCell(3);
				cell.style.width = '30px';
				bn = util.createButton('>', bnStyle, function() {
					events.turnMonth(1);
				});
				cell.appendChild(bn);
				return table;
			...
			this.getWeekDayPanel = function() {
				var table = util.createTable();
				style = 'width:23px; height:20px; cursor:pointer; border-right:1px solid #EEEEEE;border - bottom: 1 px solid# EEEEEE;line - height: 16 px;';
				var panelId = 0;
				var row = null;
				for(var m = 0; m < 6; m++) {
				...
				var style = 'height:23px; border-top:0px; border-bottom:1px solid #008FEE; borderright:0px;border - left: 0 px;background - color: #FFFFFF;';
				for(var i = 0; i < 7; i++) {
			...
			_style = 'position:absolute; width:171px; height:190px;z-index:20000; font-size:12 px;display: none;background - color: #FFFFFF;';
			util.setObjectStyle(div, _style);
	...
	JTC.setday = function(args) {
		...
		if(args) {
			...
			if(args.readOnly === true) {
				x.outObject.readOnly = true;
			}
			if(args.startDay) {
				x.startDay = util.setDate(args.startDay);
			}
		}
		var pos = {
			top: obj.offsetTop,
